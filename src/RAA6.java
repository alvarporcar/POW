...
public class RAA6 extends AbstractTableModel implements Parametres, RXXX{
  ...
  public RAA6(String sAno, String sAnoFinal) {
        int files;
        int cols;


       
        String query = " SELECT oficinas.descripcion as ofisam,ano,count(distinct(ayto)) as total";
        query += " FROM asist_admva,oficinas";
        query += " WHERE ((ano BETWEEN \"" +sAno+ "\" AND \""+sAnoFinal+"\") AND (oficinas.codigo=asist_admva.oficina))";
