...
public class RVU5 extends AbstractTableModel implements Parametres, RXXX {
  ...
  public RVU5(String sAno, String sAnoFinal) {
        int files;
        int cols;


       
        String query = " SELECT oficinas.descripcion as ofisam,ano,sum(pubs)+sum(privs) as total";
        query += " FROM vu,oficinas";
        query += " WHERE ((ano BETWEEN \"" +sAno+ "\" AND \""+sAnoFinal+"\") AND (oficinas.codigo=vu.oficina))";
        query += " GROUP BY ano,oficina";
        query += " ORDER BY ano,oficina";      
